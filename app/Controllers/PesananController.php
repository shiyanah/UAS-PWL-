...
use App\Models\TransactionModel; // Gunakan TransactionModel
use App\Models\TransactionDetailModel; // Gunakan TransactionDetailModel
use App\Models\ProductModel; // Opsional: jika perlu detail produk
...
class PesananController extends BaseController
{
    ...
    {
        helper('number'); // Untuk number_to_currency
    ...
    public function detail($id_pesanan)
    {
        ...
        $detailPesanan = $this->transactionDetailModel
            ->select('transaction_detail.*, product.foto') // DISESUAIKAN: tabel singular 'transaction_detail'
            ->join('product', 'transaction_detail.product_id = product.id', 'left') // Asumsi nama tabel produk adalah 'product'
    ...
    public function updateStatus($id_pesanan)
    {
        ...
        $validStatuses = ['menunggu pembayaran', 'diproses', 'dikirim', 'selesai', 'dibatalkan'];

        if (!in_array($status_baru, $validStatuses)) {
            return $this->response->setJSON(['status' => 'error', 'message' => 'Status tidak valid.']);
        }

        $this->transactionModel->update($id_pesanan, ['status_pesanan' => $status_baru]); // DISESUAIKAN: menggunakan 'status_pesanan'
